  ...
  pasteArray.forEach((char,index)=>{
    ...
    const handleKeyDown = (e, index) => {
      if (e.key === 'Backspace' && e.target.value === '' && index > 0) {
        inputRefs.current[index - 1].focus();
        inputRefs.current[index - 1].value = ''; // Optional: clear previous input
      }
    }
...
/*const onSubmitHandler = async (e) => {
  try {
    ...
    const userId = Cookies.get('pendingUserId'); // ✅ Grab from cookies
    ...
    if (data.success) {
      ...
      Cookies.remove('pendingUserId'); // ✅ Clean up
      getUserData();
      navigate('/');
    } else {
  ...
  try {
    ...
    /*  const onSubmitHandler = async (e) => {
        try {
          ...
          const userId = localStorage.getItem("pendingUserId"); // ✅ Grab from localStorage
  ...
  return (
    ...
    <div className='flex items-center justify-center min-h-screen  bg-gradient-to-br from-blue-200 to purple-400'>
         <img
             ...
             />
             <form onSubmit={onSubmitHandler} className='bg-slate-900 p-8 rounded-lg shadow-lg shadow-lg w-96 text-sm'>
